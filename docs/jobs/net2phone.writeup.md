...
**On Angular versions (what the archive actually proves):** the Unite frontend (`Source/UniteFrontend/` → `webapp/`) first appears Feb 2018 already on `@angular/core ^4.0.2`, then upgrades to **`^5.2.10` on 2018-04-18** ("added scss and updated to angular 5") and stays on 5.2.x through the end of the history (Nov 2018). That 4→5 bump commit was authored by a teammate (`danielidt`), though you were the heaviest webapp contributor. **No Angular 2 and no v9 are in this archive** — the v9 era is 2019–2020, in a repo not on this drive. The resume's "v2→v9, −50% load" is therefore unverified; the experience bullet has been softened to "major-version Angular upgrades" accordingly. Restore the specifics only if you can source them elsewhere.
