# Takt Digital — Software Engineer

**Dates:** May 2016 – January 2017 (resume) · Miami
**Context:** Your first engineering role, right after **Ironhack** (Mar–May 2016, Miami). Takt Digital was a Miami studio building MVPs for startups.

> **Source recovered** from the external drive: `/Volumes/MIND GEM/stark tower - transfer/code/old/Takt`. The two resume projects are both here — `stream-api` (the streaming-media engine) and `fis-aapryl` (Aapryl). Both confirmed as real, substantial work.

## 1. Streaming-media workflow engine — `stream-api` (the "sole developer / through 2nd YC round" project)
...
## 2. Aapryl — `fis-aapryl` (fintech analytics dashboard)
...
## Other Takt-era projects on the drive

`body-cam` (Android video-upload / MediaRecorder + AWS SDK), `hiike`, `happy`, `feeel`, `serverless-test` — smaller client/experiment work.

## Suggested resume bullets (upgraded with real detail)

- As an early engineer at a Miami studio, built a **serverless streaming-media pipeline** (AWS Lambda + Bitmovin video transcoding) as primary developer — ~430 commits — carrying it from MVP through a startup's second YC round.
- Delivered the **Aapryl** fintech analytics dashboard MVP end-to-end (Angular + Node.js), ~150 commits over the final stretch of the engagement.
