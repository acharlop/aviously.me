  ...
  return (
    <div className='fixed inset-0 z-50 flex flex-col bg-[var(--bg)]'>
      <div className='flex items-center justify-between gap-3 border-b border-[var(--line)] px-4 py-2'>
        <button type='button' onClick={onBack} className='text-sm text-[var(--faint)] hover:text-[var(--accent)]'>
          ← Lessons
        </button>
        <span className='min-w-0 flex-1 truncate text-center text-sm font-medium'>{lesson.title}</span>
